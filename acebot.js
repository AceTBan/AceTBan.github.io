...
function redirect(type) {
  ...
  if (type === "linkedin") {
    botMessage(
      "Pas de souci, c'est moi AceBot à votre service!! Je vous envoie vers son LinkedIn .",
    );
    // L’action est décalée APRÈS la réponse du bot
    setTimeout(() => {
      window.open(
        "https://www.linkedin.com/in/david-esteban-31-toulouse/",
        "_blank",
      );
    }, 4000);
  }
  ...
  if (type === "github") {
    botMessage(
      "Redirection vers son GitHub imminent, c'est moi AceBot à votre service !!",
    );
    // L’action est décalée APRÈS la réponse du bot
    setTimeout(() => {
  ...
  if (type === "mail") {
    botMessage(
      "Vous pouvez dès à présent lui écrire un mail. À votre service !!",
    );
    // L’action est décalée APRÈS la réponse du bot
    setTimeout(() => {
  ...
  if (type === "cv") {
    ...
    setTimeout(() => {
      window.open(
        "https://drive.google.com/file/d/1ivZ9LWlDYNaaTuh6DVKEI9nkwzju3__X/view?usp=sharing",
        "_blank",
      );
    }, 4000);
  }
  ...
  if (type === "objectif") {
    ...
    setTimeout(() => {
      showOptions([
        ...
        {
          label: "En savoir plus sur son parcours.",
          action: () => redirect("linkedin"),
        },

        // Redirection vers les projets
        {
          label: "Voir d'anvantage ses projets.",
          action: () => redirect("github"),
        },
  ...
  if (type === "skills") {
    ...
    botMessage(
      "Voici un aperçu des compétences techniques actuelles, qui sont en perpétuelle progression :",
    );
...
Son âge n’est donc pas un frein à l’alternance, bien au contraire . 
    ...
    setTimeout(() => {
      showOptions([
        ...
        { label: "Avantages RQTH", action: () => redirect("avantages_rqth") },
        {
          ...
          action: () => redirect("objectif"),
        },
        {
          label: "Posture professionnel",
          action: () => redirect("posture_professionnel"),
        },
        { label: "Accueil", action: () => startAceBot() },
      ]);
    }, 2000);
  ...
  if (type === "rqth") {
    botMessage(
      "La RQTH est une reconnaissance officielle qui permet d'accéder à certains dispositifs .",
    );
  ...
  `);
    ...
    setTimeout(() => {
      showOptions([
        ...
        //    { label: " ", action: () => redirect(" ") },
        {
          label: "En attendant un contrat, que fait-il ?",
          action: () => redirect("activite_actuelle"),
        },
        { label: "Accueil", action: () => startAceBot() },
      ]);
    }, 800);
...
window.easter = function(commande) {
    if (commande === "wouaf" || commande === "easter" || commande === "egg") {
        try {
            const badgeEgg = document.getElementById('badge-egg');
            if (badgeEgg && !badgeEgg.classList.contains('active')) {
                badgeEgg.classList.add('active');
                const compteur = document.getElementById('compteur');
                if (compteur) {
                    let actuel = parseInt(compteur.innerText) || 0;
                    compteur.innerText = actuel + 1;
                }
                return "Succès déverrouillé ! ";
            } else {
